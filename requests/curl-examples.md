...
Estos ejemplos continúan funcionando, pero los parámetros por nombre y el endpoint de provincias
están deprecados. Los clientes nuevos deberían descubrir y enviar `areaId`.
...
curl --silent --show-error --get "${BASE_URL}/statistics/fuel-prices/current" \
  ...
  --data-urlencode 'area=Málaga'
```

### Municipio por nombre y provincia padre
...
curl --silent --show-error --get "${BASE_URL}/statistics/fuel-prices/current" \
  ...
  --data-urlencode 'area=Marbella' \
  --data-urlencode 'province=Málaga'
```

### Ranking antiguo de provincias

```bash
curl --silent --show-error --get "${BASE_URL}/statistics/fuel-prices/provinces" \
  --header 'Accept: application/json' \
  --data-urlencode "countryCode=${COUNTRY_CODE}" \
  --data-urlencode "productType=${PRODUCT_TYPE}"
```
