...
public record FindStationsResult(List<FindStationsItem> items, FindStationsPage page) {
    public FindStationsResult {
        items = List.copyOf(items);
    }

    public static FindStationsResult empty(FindStationsPageRequest pageRequest) {
        return new FindStationsResult(
                List.of(),
                new FindStationsPage(pageRequest.page(), pageRequest.size(), 0, false, pageRequest.page() > 0));
    }
}
