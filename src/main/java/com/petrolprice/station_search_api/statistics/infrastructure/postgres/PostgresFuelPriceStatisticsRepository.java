...
public class PostgresFuelPriceStatisticsRepository
    ...
    @Override
    public int backfillMissingScopes() {
        String sql =
                """
                WITH country_snapshots AS (
                    SELECT snapshot_id, country, product_type, calculated_at
                    FROM fuel_price_statistics
                    WHERE normalized_locality_name IS NULL
                      AND admin_area_1_name IS NULL AND admin_area_2_name IS NULL
                      AND admin_area_3_name IS NULL
                      AND (
                          NOT EXISTS (
                              SELECT 1 FROM fuel_price_statistics locality
                              WHERE locality.snapshot_id = fuel_price_statistics.snapshot_id
                                AND locality.country = fuel_price_statistics.country
                                AND locality.product_type = fuel_price_statistics.product_type
                                AND locality.normalized_locality_name IS NOT NULL
                          )
                          OR NOT EXISTS (
                              SELECT 1 FROM fuel_price_statistics area
                              WHERE area.snapshot_id = fuel_price_statistics.snapshot_id
                                AND area.country = fuel_price_statistics.country
                                AND area.product_type = fuel_price_statistics.product_type
                                AND (area.admin_area_1_name IS NOT NULL
                                  OR area.admin_area_2_name IS NOT NULL
                                  OR area.admin_area_3_name IS NOT NULL)
                          )
                      )
                ), source AS (
                    SELECT hp.snapshot_id, s.country, hp.product_type, hp.station_id, hp.price,
                           ...
                           location.admin_area_2_name, location.admin_area_3_name
                    FROM historical_product_price hp
                    JOIN country_snapshots snapshot
                      ON snapshot.snapshot_id = hp.snapshot_id
                     AND snapshot.product_type = hp.product_type
                    JOIN station s ON s.id = hp.station_id AND s.country = snapshot.country
                    LEFT JOIN LATERAL (
                        SELECT sl.normalized_locality_name, sl.admin_area_1_name,
                               sl.admin_area_2_name, sl.admin_area_3_name
                        FROM search_location sl
                        WHERE sl.country_code = s.country
                          AND sl.normalized_postal_code = UPPER(
                              REGEXP_REPLACE(s.postal_code, '[^A-Za-z0-9]', '', 'g'))
                        ORDER BY similarity(
                            sl.normalized_locality_name,
                            LOWER(COALESCE(s.municipality, s.locality, ''))
                        ) DESC, sl.accuracy DESC NULLS LAST
                    ...
                    SELECT snapshot_id, country, product_type, normalized_locality_name,
                           admin_area_1_name, admin_area_2_name, admin_area_3_name,
                           AVG(price) average_price, MIN(price) minimum_price, MAX(price) maximum_price,
                           COUNT(*) station_count,
                           (ARRAY_AGG(station_id ORDER BY price, station_id))[1] cheapest_station_id,
                           (ARRAY_AGG(station_id ORDER BY price DESC, station_id))[1] most_expensive_station_id
                    FROM scopes
                    GROUP BY snapshot_id, country, product_type, normalized_locality_name,
                             admin_area_1_name, admin_area_2_name, admin_area_3_name
                )
                ...
                SELECT gen_random_uuid(), a.snapshot_id, a.country, a.product_type,
                       a.normalized_locality_name, a.admin_area_1_name, a.admin_area_2_name,
                       a.admin_area_3_name, a.average_price, a.minimum_price, a.maximum_price,
                       a.station_count, a.cheapest_station_id, a.most_expensive_station_id,
                       snapshot.calculated_at
                FROM aggregates a
                JOIN country_snapshots snapshot
                  ON snapshot.snapshot_id = a.snapshot_id
                 AND snapshot.country = a.country
                 AND snapshot.product_type = a.product_type
