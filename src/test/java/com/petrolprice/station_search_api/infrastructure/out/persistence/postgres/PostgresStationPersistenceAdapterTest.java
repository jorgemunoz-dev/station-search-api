...
class PostgresStationPersistenceAdapterTest {

    @Mock
    private PostgresJPAStationRepository jpaStationRepository;

    @Mock
    private StationEntityMapper mapper;

    @InjectMocks
    PostgresStationPersistenceAdapter adapter;
    ...
    void shouldReplaceOpeningPeriodsWhenTheyChanged() {
        ...
        when(mapper.mapOpeningPeriodsToDomain(currentOpeningPeriods))
            .thenReturn(currentDomainOpeningPeriods);

        when(stationSnapshot.getOpeningPeriods())
            .thenReturn(snapshotOpeningPeriods);

        when(jpaStationRepository.findByExternalIdAndCountry("4375", Country.ES))
            .thenReturn(Optional.of(existingEntity));

        when(mapper.mapOpeningPeriods(snapshotOpeningPeriods))
            .thenReturn(newOpeningPeriods);
